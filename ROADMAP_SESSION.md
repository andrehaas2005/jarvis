...
- **Frontend:** `login.html`/`login.css`/`login.js` seguindo o mockup fornecido pelo usuário. Logo final: arte oficial fornecida pelo usuário (`assets/brand/jarvis-logo.png`, engrenagem + perfil digital), não o SVG desenhado à mão da primeira versão — favicons (`assets/brand/favicon-{16,32,64,128}.png`) gerados da mesma arte. `index.html` redireciona pra `login.html` se não houver token válido no `localStorage`; topbar do HUD mostra o nome de quem logou + botão de logout.
- **Bug achado no deploy:** `Dockerfile.frontend` fazia `COPY` de uma lista explícita de arquivos — não incluía `login.html/css/js` nem `assets/brand/`. Build "sucesso" mas a imagem não tinha os arquivos novos (login sem estilo/logo em produção). Fix antes de virar visível: adicionado ao `COPY`.
...
- **✅ Deployado em produção:** volume `/root/jarvis-data:/app/data` adicionado ao `jarvis-backend` no `docker-compose.yml` do servidor (persiste o SQLite entre rebuilds); `JARVIS_AUTH_SECRET`/`JARVIS_DB_PATH` no `/root/.env`; `jarvis-backend` e `jarvis-frontend` rebuildados; Cloudflare cache purgado (`jarvis-hud.andre.haas.nom.br`).
- Testado real: local (curl + browser — login errado/certo, `/auth/me`, logout, mobile 375px) **e produção** (`curl https://jarvis-api.andre.haas.nom.br/auth/login` retornando token+user; HUD em `https://jarvis-hud.andre.haas.nom.br` mostrando a tela de login com a logo nova; edge do Cloudflare confirmado servindo o `script.js` atualizado via fetch com bypass de cache).
